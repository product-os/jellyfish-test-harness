...
module.exports.before = async (test, plugins = [], cards = []) => {
	utils.loadPlugins(test, plugins)

	await workerHelpers.worker.before(test, {
...
module.exports.after = async (test) => {
	await workerHelpers.worker.after(test)
}

module.exports.afterEach = async (test) => {
	nock.cleanAll()
	await module.exports.restore(test)
}

module.exports.restore = async (test) => {
...
module.exports.save = async (test) => {
	await test.context.jellyfish.backend.connection.any('CREATE TABLE cards_copy AS TABLE cards')
}

module.exports.run = async (ava, suite) => {
	const avaTest = ava.serial || ava

	avaTest.before(async (test) => {
		await module.exports.before(test, suite.plugins, suite.cards)
		if (suite.before) {
			await suite.before(test)
		}
		await module.exports.save(test)
	})

	avaTest.beforeEach(async (test) => {
		if (suite.beforeEach) {
			await suite.beforeEach(test)
		}
	})

	avaTest.after.always(async (test) => {
		if (suite.after) {
			await suite.after(test)
		}
		await module.exports.after(test)
	})

	avaTest.afterEach.always(async (test) => {
		if (suite.afterEach) {
			await suite.afterEach(test)
		}
		await module.exports.afterEach(test)
	})
	...
	for (const testCaseName of Object.keys(suite.scenarios)) {
		...
		for (const variation of getVariations(testCase.steps, {
			...
			avaTest(`(${variation.name}) ${testCaseName}`, async (test) => {
				if (suite.pre) {
